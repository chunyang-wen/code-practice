...
def sum():
    ...
    for line in fd:
        # exp_id scale luw pv1 pv2 click gain
        str1 = line.split('\t')
        start_ind = 3
        if int(str1[start_ind]) == 0 or int(str1[start_ind+1]) == 0:
            continue
        l_pv1 = float(str1[start_ind])
        l_pv2 = float(str1[start_ind+1])
        pv1 += l_pv1
        pv2 += l_pv2
        l_click = float(str1[start_ind+2])
        l_gain  = float(str1[start_ind+3])
        click += l_click
        gain  += l_gain
